...
#将source的parameters和buffers复制到targets和它的后代中
...
def logits_greedy(state, info, logits, height, width):
    ...
    snakes_positions = {key: state[key] for key in state.keys() & {1, 2}}
    snake_map = make_grid_map(width, height, beans_positions, snakes_positions)
    state = np.array(snake_map)
    state = np.squeeze(snake_map, axis=2)
    ...
    logits = torch.Tensor([logits]).to(device)
    logits_action = [out for out in logits]
