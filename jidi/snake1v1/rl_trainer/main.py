...
def main(args):
    ...
    while episode < args.max_episodes:
        ...
        while True:
            ...
            # logits and radom
            #actions = logits_random(action_dim, action)
            # logits and greedy
            actions = logits_greedy(get_state(state),env,action,env.board_height,env.board_width)
            ...
            if args.episode_length <= step or (True in done):
                ...
                if episode % args.save_interval == 0:
                    print(f'[Episode {episode:05d}] avarage_score: {np.mean(sum_score)} avarage_reward: {np.mean(sum_reward):.2f}',end=" ")
                else:
                    sum_score.append(episode_reward[0])
                    sum_reward.append(step_reward[0])
                reward_tag = 'reward'
                loss_tag = 'loss'
                ...
                if model.loss:
                    ...
                    if episode % args.save_interval == 0:
                        print(f'\t\t\t\tloss {np.mean(sum_loss):.3f}')
                        sum_score = []
                        sum_reward = []
                        sum_loss = []
                    else:
                        sum_loss.append(model.loss)
