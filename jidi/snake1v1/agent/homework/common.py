...
def get_observations(state, agent_trained_index, obs_dim):
    ...
    snakes_other_position = np.array(snakes_positions[5-agents_index], dtype=object) # todo
    observations[0][16:] = snakes_other_position[0][:]

    return observations
#判断action是否安全
# 方向[0,1,2,3]分别表示[上，下，左，右]
def check_action(observation,action,action1):
    index = observation["controlled_snake_index"]
    index1 = 5-index
    snake = observation[index]
    snake1 = observation[index1]
    width = observation["board_width"]
    height = observation["board_height"]
    obs = get_observations(observation, 0, 18)
    surrrounding = obs[0][2:6]
    enaction = [i for i, x in enumerate(surrrounding) if x in [0, 1]]
    next_head = move_head(snake[0],action,width,height)
    next_head1 = move_head(snake1[0],action1,width,height)
    snake.pop()
    snake.insert(0, next_head)
    snake1.pop()
    snake1.insert(0,next_head1)
    obs1 = get_observations(observation, 0, 18)
    surrrounding1 = obs1[0][2:6]
    enaction1 = [i for i, x in enumerate(surrrounding1) if x in [0, 1]]
    if next_head in snake1 or not len(enaction1):
        if(action in enaction):
            enaction.remove(action)
        if(len(enaction)):
            action = random.choice(enaction)
            #action = check_action(observation,action,action1)
            return action
        else:
            return action
    else:
...
def move_head(head,action,width, height):
    head1=copy.deepcopy(head)
    if action == 0:
        head1[0]=(head1[0] - 1) % height
    elif action == 1:
        head1[0] = (head1[0] + 1) % height
    elif action == 2:
        head1[1] = (head1[1] - 1) % width
    else:
        head1[1] = (head1[1] + 1) % width
    return head1
